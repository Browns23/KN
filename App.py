...
# ─────────────────────────────────────────────────────────────
# Helper functions + regex (Modified for the new invoice structure)
# ─────────────────────────────────────────────────────────────
...
# Pattern for chargeable weight (often listed separately)
CHARGEABLE_KG_PAT = re.compile(
    r"CH\.?\s*W(?:EIGHT)?\s*[:\-]?\s*([\d,.]+)\s*(KG|KGS?|LB|M3|CBM)", re.I
)

# Pattern for Air Freight *amount* (assuming a typical charge line structure)
AIR_FRT_AMOUNT_PAT = re.compile(
    r"AIRFREIGHT\s+CHARGE.*?\s+([\d,]+\.\d{2})\s*(?:[A-Z]{3}|USD|EUR|CAD)", re.I
)
SEA_FRT_AMOUNT_PAT = re.compile(
    r"(?:SEA|OCEAN)\s*FREIGHT\s+CHARGE.*?\s+([\d,]+\.\d{2})\s*(?:[A-Z]{3}|USD|EUR|CAD)", re.I
)
...
def parse_invoice_pdf_bytes(data: bytes, filename: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # --- 6. Freight Mode and Amount ---
        f_mode = f_amount = None

        m = AIR_FRT_AMOUNT_PAT.search(text)
        if m:
            f_mode, f_amount = "Air", _f(m.group(1))
        else:
            m = SEA_FRT_AMOUNT_PAT.search(text)
            if m:
                f_mode, f_amount = "Sea", _f(m.group(1))
        ...
        return {
            ...
            "Subtotal": subtotal,
            "Freight_Mode": f_mode,
            "Freight_amount": f_amount, # Note: this is now 'Freight_amount'
        }
    except Exception:
